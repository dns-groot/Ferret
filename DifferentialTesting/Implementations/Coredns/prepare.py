...
def run(zone_file, zone_domain, cname, port, restart, tag):
    ...
    subprocess.run(['docker', 'cp', 'Corefile_' + cname, cname +
                    ':/go/coredns/Corefile'], stdout=subprocess.PIPE, check=True)
