...
def run(zone_file, zone_domain, cname, port, restart, tag):
    ...
    if restart:
        ...
        subprocess.run(['docker', 'run', '-dp', str(port)+':53/udp', '--name=' +
                        cname, 'knot' + tag], stdout=subprocess.PIPE, check=True)
    else:
    ...
    subprocess.run(['docker', 'cp', zone_file, cname +
                    ':/usr/local/var/lib/knot/'], stdout=subprocess.PIPE, check=True)
    ...
    subprocess.run(['docker', 'cp', 'knot_'+cname+'.conf',
                    cname + ':/usr/local/etc/knot/knot.conf'], stdout=subprocess.PIPE, check=True)
    ...
    subprocess.run(['docker', 'exec', cname, 'dos2unix', '/usr/local/var/lib/knot/' +
                    zone_file.name], stdout=subprocess.PIPE, check=True)
    # Start the server
    subprocess.run(['docker', 'exec', cname, 'knotd', '-d', '-c',
                    '/usr/local/etc/knot/knot.conf'], stdout=subprocess.PIPE, check=True)
