...
def run(zone_file, _, cname, port, restart, tag):
    ...
    if restart:
        ...
        subprocess.run(['docker', 'run', '-dp', str(port)+':53/udp',
                        '--name=' + cname, 'maradns' + tag], stdout=subprocess.PIPE, check=True)
    else:
        # Stop the running server instance inside the container
        subprocess.run(['docker', 'exec', cname,
                        '/etc/init.d/maradns', 'stop'], stdout=subprocess.PIPE, check=True)
    # Copy the new zone file into the container
    subprocess.run(['docker', 'cp', zone_file,
                    cname + ':/etc/maradns'], stdout=subprocess.PIPE, check=True)
